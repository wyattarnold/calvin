"""
Annual-step perfect foresight run — solves the 82-year network with annual timesteps.

Model dir: my-models/calvin-pf-astep/
  links.csv      — annual-step network (auto-generated on first run)
...
The links file is built automatically if absent.  To regenerate it manually:
  delete my-models/calvin-pf-astep/links.csv and re-run this script.

Configuration is loaded from my-models/calvin-pf-astep/config.toml (created
automatically from scripts/configs/calvin-pf-astep.toml on first run).
"""
import os
import sys
import numpy as np
...
MODEL_DIR  = './my-models/calvin-pf-astep'
cfg = load_config('calvin-pf-astep', MODEL_DIR)

DATA_PATH        = cfg['run']['data_path']
SOLVER           = cfg['run'].get('solver', 'cbc')
NPROC            = cfg['run'].get('nproc', 1)
...
if not os.path.isfile(LINKS_FILE):
    print('Building annual-step links file...')
    from calvin.network.loader import load_network
    from calvin.network.prepare import prepare_pf_astep

    network = load_network(DATA_PATH)
    prepare_pf_astep(DATA_PATH, MODEL_DIR, network=network,
