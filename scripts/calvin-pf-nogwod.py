"""
Perfect foresight run with no groundwater overdraft — constrains end-of-period
GW storage to be at least equal to initial storage (no net GW depletion over
the 82-year period).

Model dir: my-models/calvin-pf-nogwod/
  links82yr-nogwod.csv  — constrained network (auto-generated on first run)
  results/              — output CSVs written here

The links file is built automatically if absent.  To regenerate it manually:
  delete my-models/calvin-pf-nogwod/links82yr-nogwod.csv and re-run this script.

Configuration is read from my-models/calvin-pf-nogwod/config.toml (auto-created
from scripts/configs/calvin-pf-nogwod.toml on first run).  Edit the model-dir
...
MODEL_DIR  = './my-models/calvin-pf-nogwod'
cfg        = load_config('calvin-pf-nogwod', MODEL_DIR)

DATA_PATH         = cfg['run']['data_path']
SOLVER            = cfg['run']['solver']
NPROC             = cfg['run']['nproc']
CONSTRAIN_ENDING  = cfg['network'].get('constrain_ending', 'gw')
NODE_LB_OVERRIDES = cfg['network'].get('node_lb_overrides', {})

LINKS_FILE = os.path.join(MODEL_DIR, 'links82yr-nogwod.csv')
...
if not os.path.isfile(LINKS_FILE):
    print('Building no-GW-overdraft links file...')
